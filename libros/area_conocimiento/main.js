...
<button
onclick="editarArea('${area.Id_area_conocimiento}','${area.Area_conocimiento}','${area.N_estante}')"
class="icon-btn">
...
<button
onclick="eliminarArea('${area.Id_area_conocimiento}')"
...
// VALIDAR ESTANTE NUMEROS
const estanteInput = document.getElementById("numeroEstante");

estanteInput.addEventListener("input", () => {
  estanteInput.value = estanteInput.value.replace(/[^0-9]/g, "");

  if (estanteInput.value.length > 10) {
    estanteInput.value = estanteInput.value.slice(0, 10);
  }
});
